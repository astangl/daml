...
private[lf] final class Compiler(
  ...
  private[this] def compile(expr0: Expr): SExpr =
    expr0 match {
      ...
      case EEnumCon(tyCon, constructor) =>
        val enumDef =
          lookupEnumSignature(tyCon).getOrElse(throw CompilationError(s"enum $tyCon not found"))
        SEValue(SEnum(tyCon, constructor, enumDef.constructorRank(constructor)))
      case EVariantCon(tapp, variant, arg) =>
        val variantDef = lookupVariantSignature(tapp.tycon)
        ...
        case CaseAlt(pat, expr) =>
          pat match {
            case CPVariant(tycon, variant, binder) =>
              val variantDef = lookupVariantSignature(tycon).getOrElse(
            ...
            case CPEnum(tycon, constructor) =>
              val enumDef = lookupEnumSignature(tycon).getOrElse(
  ...
  private[this] def lookupVariantSignature(tycon: TypeConName): Option[DataVariant] =
    lookupDefinitionSignature(tycon).flatMap {
      case DDataTypeSignature(_, _, data: DataVariant) =>
        Some(data)
      case _ =>
        None
    }

  private[this] def lookupEnumSignature(tycon: TypeConName): Option[DataEnum] =
    lookupDefinitionSignature(tycon).flatMap {
      case DDataTypeSignature(_, _, data: DataEnum) =>
        Some(data)
      case _ =>
        None
    }
  ...
  private[this] def lookupRecordIndex(tapp: TypeConApp, field: FieldName): Int =
    lookupDefinitionSignature(tapp.tycon)
      .flatMap {
        case DDataTypeSignature(_, _, DataRecordSignature(fields, _)) =>
