...
private[platform] class FlywayMigrations(jdbcUrl: String)(implicit loggingContext: LoggingContext) {
  ...
  def validate()(implicit context: Context): Future[Unit] =
    dataSource.use { ds =>
      Future {
  ...
  def migrate(allowExistingSchema: Boolean = false)(implicit context: Context): Future[Unit] =
    dataSource.use { ds =>
      Future {
  ...
  def reset()(implicit context: Context): Future[Unit] =
    dataSource.use { ds =>
      Future {
        val flyway = configurationBase(dbType).dataSource(ds).load()
